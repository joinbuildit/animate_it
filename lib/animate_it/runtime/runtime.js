...
(function (global) {
  ...
  function boot() {
    ...
    waitForReady(document)
      .then(function () {
        delete document.documentElement.dataset.animateItError;
        document.documentElement.dataset.animateItReady = "1";
        emit("ready", { manifest: manifest, chapter: currentChapterName });
      })
      .catch(function (error) {
        var message = error && error.message ? error.message : "AnimateIt player failed readiness";
        document.documentElement.dataset.animateItError = message;
